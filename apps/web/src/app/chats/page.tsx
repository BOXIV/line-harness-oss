...
import FriendInfoPanel from '@/components/chats/friend-info-panel'
import {
  detectFriendSource,
  isUnlinkedFriend,
  SOURCE_LABELS,
...
export default function ChatsPage() {
  ...
  /** xl 未満でユーザー情報をドロワーとして開いているか。xl 以上では常時右カラムに出す。 */
  ...
  /**
   * 撮影日程調整の招待を LINE で送る。
   * 顧客名・都道府県は Notion から補完するが、都道府県が無いとエリア判定ができないので
   * そのときだけ手入力にフォールバックする。
   */
  ...
  // 右カラムとドロワーで同じ中身を出すので、props は 1 箇所で組む。
  const infoPanelProps = chatDetail
    ? {
        friendId: chatDetail.friendId,
        displayLabel: formatChatLabel(chatDetail),
        pictureUrl: chatDetail.friendPictureUrl,
        lineUserId: chatDetail.lineUserId,
        // 出品者リンクと購入者リンクは両方持ち得るので、あるものを全て渡す。
        // 旧 worker（notionLinks 無し）と繋がったときは notion 1 件にフォールバック。
        links: (chatDetail.notionLinks
          ? (['seller', 'buyer'] as const).map((src) => chatDetail.notionLinks?.[src])
          : [chatDetail.notion]
        ...
        onSendScheduleInvite: handleSendScheduleInvite,
        sendingSchedule,
        notice: notionMessage || undefined,
      }
    : null
    ...
    <div>
      ...
      <div className="flex gap-4 h-[calc(100vh-120px)] lg:h-[calc(100vh-180px)]">
        ...
        {/* 顧客名が長い（会社名＋氏名＋車種）ので一覧は広めに取る。
            xl 以上では右にユーザー情報カラムが入るので、ここはこれ以上広げない。 */}
        ...
        <div className={`flex-1 bg-white rounded-lg shadow-sm border border-gray-200 flex-col overflow-hidden ${selectedChatId || selectedFriendId ? 'flex' : 'hidden lg:flex'}`}>
          {selectedFriendId && !selectedChatId ? (
            ...
            <>
              {/* Chat Header — 名前と戻る/情報だけの細いバー。
                  ステータス・Notion 連携・操作ボタンは右のユーザー情報カラムへ移した。 */}
              <div className="px-4 py-3 border-b border-gray-200 flex items-center gap-2">
                ...
                {/* xl 未満ではユーザー情報カラムを出す場所が無いのでドロワーで開く。 */}
                <button
                  onClick={() => setInfoOpen(true)}
                  className="xl:hidden flex-shrink-0 px-2 py-1 min-h-[36px] text-xs font-medium text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-md transition-colors"
                  title="ユーザー情報"
                >
                  ⓘ 情報
                </button>
        ...
        {/* ユーザー情報カラム。xl 以上ではトークの右に常駐する。 */}
        {chatDetail && (
          <aside className="hidden xl:flex w-72 flex-shrink-0 bg-white rounded-lg shadow-sm border border-gray-200 flex-col overflow-hidden">
            <FriendInfoPanel {...infoPanelProps!} />
          </aside>
        )}
      </div>
      ...
      {chatDetail && infoOpen && (
        <div className="xl:hidden fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-80 max-w-[85vw] h-full bg-white shadow-2xl flex flex-col">
            <FriendInfoPanel {...infoPanelProps!} onClose={() => setInfoOpen(false)} />
          </div>
        </div>
      )}
